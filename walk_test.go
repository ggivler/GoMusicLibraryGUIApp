// This file is a port of the Python walk_test.py to Go
// Note: Go treats files ending in _test.go as test files, so this cannot be run directly with "go run"
// To use this file, either:
// 1. Rename it to something like "walk_main.go" and run with "go run walk_main.go"
// 2. Or use "go build" to create an executable
...
func (fm *FileMethods) SplitSongTitle(songTitle string) string {
	...
	splitSongTitle := re.ReplaceAllString(songTitle, " ")
	
	fmt.Printf("Split song title: %s\n", splitSongTitle)
	return splitSongTitle
}
